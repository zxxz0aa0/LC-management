...
| 備份類型 | 執行頻率 | 保留期限 | 說明 |
|---------|---------|---------|------|
| 自動備份 | 每日 3 次 (02:00、12:00、18:00) | 60 天 | 完整資料庫備份（使用 --single-transaction 不鎖表） |
| 手動備份 | 依需求執行 | 永久保存 | 重大更新前手動備份 |

**備份時間說明**：
- **凌晨 02:00**：系統閒置時段，主要完整備份
- **中午 12:00**：午間備份，保護上午工作資料
- **晚上 18:00**：下班備份，保護全天工作資料
- **RPO (復原點目標)**：最多遺失 6-8 小時資料

### 1.3 備份保留政策
```
storage/backups/
├── daily/          # 自動備份 (保留 60 天，每日 3 次)
│   ├── lc_management_2025_11_21_020000.sql.gz  # 凌晨 02:00
│   ├── lc_management_2025_11_21_120000.sql.gz  # 中午 12:00
│   ├── lc_management_2025_11_21_180000.sql.gz  # 晚上 18:00
│   ├── lc_management_2025_11_22_020000.sql.gz
│   ├── lc_management_2025_11_22_120000.sql.gz
│   └── lc_management_2025_11_22_180000.sql.gz
└── manual/         # 手動備份 (永久保存)
...
**保留策略說明**：
- 每日產生 3 個備份檔案（02:00、12:00、18:00）
- 總共保留 60 天 × 3 = 180 個備份檔案
- 自動清理超過 60 天的舊備份
- 手動備份不受自動清理影響，永久保存
...
forfiles /p "%BACKUP_DIR%" /m *.sql* /d -60 /c "cmd /c del @path" 2>nul
if !ERRORLEVEL! equ 0 (
...
# 手動快速備份 (Windows) - 使用 --single-transaction 不鎖表
cd C:\xampp\mysql\bin
mysqldump -uroot --single-transaction lc_management > C:\xampp\htdocs\LC-management\storage\backups\manual\backup_%date:~0,10%.sql
```
...
```bash
# 清理舊備份 (超過 60 天)
find storage/backups/daily -name "*.sql*" -mtime +60 -delete

# 清理舊備份 (Windows)
forfiles /p "C:\xampp\htdocs\LC-management\storage\backups\daily" /s /m *.sql* /d -60 /c "cmd /c del @path"
...
| 1-5GB | 5-15 分鐘 | 02:00, 12:00, 18:00 | 避開高峰時段 |
...
**每日 3 次備份的儲存空間需求**:
- 單次備份大小（壓縮後）：預估 200-600MB
- 每日 3 次備份：600MB-1.8GB
- 60 天保留期限：36GB-108GB
- **建議預留儲存空間**: 至少 150GB
...
| 備份檔案 | 60 天 | 12 個月 | 業務連續性需求 |

**備份保留策略（符合上述要求）**:
- 每日備份：60 天
- 手動備份：永久保存（建議重大更新前手動備份並歸檔）
